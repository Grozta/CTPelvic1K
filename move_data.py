...
if __name__ == '__main__':
    ...
    assert 0 == len(os.listdir(pre_data_path))
    ...
    for dataset_name, v in data_inform.items():
        ...
        image_train_list = target_img_list
        label_train_list = label_list
        ...
        for image_filename, label_filename in tqdm(zip(image_train_list, label_train_list)):
            ...
            shutil.copyfile(image_filename, os.path.join(pre_data_path, f'dataset{index}_{file_name}_data.nii.gz'))
            shutil.copyfile(label_filename, os.path.join(pre_data_path, prefix + os.path.basename(label_filename)))

    # check
    nii_files_tr_data = subfiles(pre_data_path, True, None, "_data.nii.gz", True)
    nii_files_tr_seg = subfiles(pre_data_path, True, None, "_mask_4label.nii.gz", True)
    print('checking')
