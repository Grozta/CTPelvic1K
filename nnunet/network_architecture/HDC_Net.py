...
from nnunet.training.network_training.sync_batchnormal import SynchronizedBatchNorm3d
...
class HDC_Net(nn.Module):
    ...
    def predict_3D(self, x, do_mirroring: bool, num_repeats=1, use_train_mode=False, batch_size=1,
                   ...
                   pad_border_mode="edge", pad_kwargs=None, all_in_gpu=True):
        """
        :param x: (c, x, y , z)
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x, num_repeats, BATCH_SIZE=None, tile_in_z=True, step=2,
        ...
        with torch.no_grad():
            ...
            with tqdm(total=len(xsteps) * len(ysteps) * len(zsteps)) as p_bar:
                for x in xsteps:
