...
def augment_mirroring(sample_data, sample_seg=None, axes=(0, 1, 2)):
    ...
    idx = 0
    assert idx == 0, 'some wrongs happening...'
...
def mirror_aug_in_dataloading(data, seg):
    ...
    for b in range(data.shape[0]):
        data_b = data[b]
        seg_b = seg[b]
        ret_data, ret_seg = augment_mirroring(data_b, seg_b, axes=axes)
        new_data.append(ret_data)
        new_seg.append(ret_seg)
    new_data = np.stack(new_data, 0)
    new_seg = np.stack(new_seg, 0)
...
def unpack_dataset(folder, threads=48, key="data"):
    """
        ...
        for j, i in enumerate(selected_keys):
            ...
            with open(self._data[i]['data_file'][:-4] + ".pkl", 'rb') as f:
                try:
                    ...
                    reasonable_crops = None

            if not force_fg or reasonable_crops is None:
                if np.random.rand() < 0.5 or reasonable_crops is None:
                    lb_x = - need_to_pad[0] // 2
                    ub_x = shape[0] + need_to_pad[0] // 2 + need_to_pad[0] % 2 - self.patch_size[0]
                    lb_y = - need_to_pad[1] // 2
                    ub_y = shape[1] + need_to_pad[1] // 2 + need_to_pad[1] % 2 - self.patch_size[1]
                    ...
                    bbox_x_lb = np.random.randint(reasonable_crops[0][0] - self.patch_size[0] // 2,
                                                  reasonable_crops[0][1] - self.patch_size[0] // 2)
                    bbox_y_lb = np.random.randint(reasonable_crops[1][0] - self.patch_size[1] // 2,
                                                  reasonable_crops[1][1] - self.patch_size[1] // 2)
                    bbox_z_lb = np.random.randint(reasonable_crops[2][0] - self.patch_size[2] // 2,
                                                  reasonable_crops[2][1] - self.patch_size[2] // 2)
            else:
                foreground_classes = np.unique(case_all_data[1, :, :, :])
                selected_class = np.random.choice(foreground_classes)
                ...
                # a class can get lost in the preprocessing. This is not a problem at all because that segmentation was
                # most likely faulty anyways but it may lead to crashes here.
                if len(index_of_that_class) != 0:
                    selected_voxel = voxels_of_that_class[np.random.choice(index_of_that_class)]
                    bbox_x_lb = max(reasonable_crops[0][0] - self.patch_size[0] // 2,
                                    selected_voxel[0] - self.patch_size[0] // 2)
                    bbox_y_lb = max(reasonable_crops[1][0] - self.patch_size[1] // 2,
                                    selected_voxel[1] - self.patch_size[1] // 2)
                    bbox_z_lb = max(reasonable_crops[2][0] - self.patch_size[2] // 2,
