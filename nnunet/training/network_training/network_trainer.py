...
from datetime import datetime
from nnunet.training.loss_functions.ND_Crossentropy import get_converted_sdf_from_target, get_sdf_from_target
...
class NetworkTrainer(object):
    ...
    def load_latest_checkpoint(self, train=True):
        if isfile(join(self.output_folder, "model_final_checkpoint.model")):
            print('loading \"model_final_checkpoint.model\" for validation...')
            return self.load_checkpoint(join(self.output_folder, "model_final_checkpoint.model"), train=train)

        if isfile(join(self.output_folder, "model_latest.model")):
            print('loading \"model_latest.model\" for validation...')
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        sdf_heatmap = None
        ### sdf ###
        # target_raw = target.clone().numpy().astype('uint16')
        # heatmap = get_sdf_from_target(target_raw)
        ...
        if not isinstance(sdf_heatmap, torch.Tensor) and not sdf_heatmap is None:
            sdf_heatmap = torch.from_numpy(sdf_heatmap).float()

        data = data.cuda(non_blocking=True)
        target = target.cuda(non_blocking=True)
        if not sdf_heatmap is None:
            sdf_heatmap = sdf_heatmap.cuda(non_blocking=True)
        ...
        if run_online_evaluation:
            self.run_online_evaluation(output if not isinstance(output, tuple) else output[0], target)
    ...
    def update_train_loss_MA(self):
        if self.train_loss_MA is None:
            ...
            self.train_loss_MA = self.train_loss_MA_alpha * self.train_loss_MA + \
